...
import { FaCheckCircle, FaClipboardList, FaFireAlt } from "react-icons/fa";
...
export default function Greeing() {
  ...
  return (
    <div className="p-6 md:p-10 text-white  min-h-screen space-y-8">
      {/* Assistant Selector */}
      <div>
        <select
          value={selectedAssistant}
          onChange={(e) => setSelectedAssistant(e.target.value)}
          className="bg-[#1f2335] text-white px-4 py-2 rounded-lg border border-white/10"
        >
      ...
      <div>
        ...
        <p className="text-gray-400 mt-1">Here’s your day at a glance:</p>
      </div>

      {/* Top Grid */}
      <div className="grid grid-cols-5 grid-rows-6 gap-6">
        {/* 1. Performance (div1) - ซ้ายบน */}
        <div className="col-span-3 row-span-3 bg-[#181b2b] rounded-xl p-4">
          <p className="text-sm text-gray-400 mb-2">Performance / 7 days</p>
        ...
        <div className="col-start-4  row-span-3 bg-[#181b2b] rounded-xl px-4 py-6 flex flex-col items-center justify-center text-white">
          <p className="text-xs text-gray-400 mb-3">Total Tasks</p>
          <h2 className="text-5xl font-extrabold">15</h2>
        </div>
        {/* 3. Task Details (div3) - ขวาสุดแนวตั้งเต็ม */}
        <div className="col-start-5 row-span-6 bg-[#181b2b] rounded-xl px-4 py-6 text-white text-center">
          <p className="text-xs text-gray-400 mb-6">Task Details</p>
        ...
        <div className="col-span-4 row-start-4 row-span-3 bg-[#009CA9] rounded-2xl p-6 text-white">
          {/* Header */}
      ...
      {/* Project List */}
      <div className="bg-[#181b2b] rounded-xl p-6 space-y-5">
        <h3 className="text-white font-semibold text-base">PROJECTLIST</h3>
        {[
          {
          ...
          <div
            key={i}
            className="flex flex-wrap items-center justify-between gap-3 bg-[#1f2335] p-4 rounded-xl"
          >
            {/* Left: Title */}
            ...
            <div className="flex flex-col items-center w-40">
              ...
              <span className="text-[10px] text-gray-300 mt-1">Progress</span>
            </div>

            {/* Right: Date + Messages */}
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-white/10 text-xs text-white px-5 py-1 rounded-full">
                <span className="w-2 h-2 rounded-full bg-red-500" />
                <span>{item.due}</span>
              </div>
              <div className="w-10 h-7 flex items-center justify-center rounded-full bg-white/10 text-white gap-1 font-semibold text-xs">
